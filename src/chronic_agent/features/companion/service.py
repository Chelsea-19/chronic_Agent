...
from chronic_agent.core.contracts import CompanionTodayOut, SummaryOut
from chronic_agent.platform.repositories import HealthRepository, MealRepository, MedicationRepository, ReminderRepository
...
class CompanionService:
    ...
    def today_view(self) -> CompanionTodayOut:
        summary = self.summary()
        meal_tags = Counter(tag for row in self.meal_repo.list_within_days(1) for tag in row.risk_tags.split('、') if tag)
        coach = '今天建议优先完成服药、补齐晨间血压与空腹血糖，并尽量避免高糖饮料和高盐外卖。'
        if summary.pending_reminders == 0:
            coach = '今天的提醒都处理完了，接下来重点观察血糖血压趋势，并保持规律饮食。'
        return CompanionTodayOut(
            date=summary.date,
            latest_blood_pressure=summary.latest_blood_pressure,
            latest_fasting_glucose=summary.latest_fasting_glucose,
    ...
    def insights(self) -> dict:
        ...
        meal_tag_counter = Counter(tag for row in week_meals for tag in row.risk_tags.split('、') if tag)
        return {
