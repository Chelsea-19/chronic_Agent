...
from chronic_agent.core.contracts import WorkflowRequest
...
from chronic_agent.platform.db import WorkflowRun
from chronic_agent.platform.repositories import AuditLogRepository, WorkflowRepository
...
WORKFLOW_DEFINITIONS = {
    'daily_review': WorkflowDefinition('daily_review', ['created', 'collect_context', 'evaluate_risk', 'generate_actions', 'completed']),
    'medication_reconciliation': WorkflowDefinition('medication_reconciliation', ['created', 'collect_medications', 'generate_reminders', 'completed']),
    'previsit_digest': WorkflowDefinition('previsit_digest', ['created', 'collect_context', 'build_digest', 'completed']),
    'adherence_followup': WorkflowDefinition('adherence_followup', ['created', 'identify_gaps', 'generate_interventions', 'completed']),
    'high_risk_escalation': WorkflowDefinition('high_risk_escalation', ['created', 'verify_condition', 'notify_clinician', 'completed']),
}


class StateMachineEngine:
    """
    Executes a workflow run step by step, handling retries and errors,
    and recording step-level execution logs.
    """
    ...
    def advance(self, run: WorkflowRun) -> None:
        ...
        try:
            ...
            self._fail(run, f"Invalid state {run.current_state} for workflow {run.workflow_type}")
            return
    ...
    def _fail(self, run: WorkflowRun, reason: str, failed_state: str = "error") -> None:
        self.repo.update(run.id, status='failed', append_log={'state': failed_state, 'error': reason, 'at': datetime.utcnow().isoformat()})
        self.audit.add(
...
class WorkflowService:
    ...
    def _register_handlers(self) -> None:
        ...
        self.engine.register_handler('medication_reconciliation', 'generate_reminders', self._handle_med_recon_reminders)
    ...
    def _handle_med_recon_reminders(self, run: WorkflowRun) -> str:
    ...
    def run(self, payload: WorkflowRequest) -> WorkflowRun:
        ...
        return self.repo.update(row.id) # refresh
