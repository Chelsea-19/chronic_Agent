...
from chronic_agent.platform.repositories import HealthRepository
from chronic_agent.core.contracts import MealAnalyzeRequest, WorkflowRequest, ReportRequest
...
class ToolRegistry:
    ...
    def _register_all_tools(self):
        ...
        self.register_tool(
            ...
            description="执行特定的自动化管理流程，如'daily_review' (每日回顾), 'medication_reconciliation' (药物核对), 'previsit_digest' (门诊前准备)。",
    ...
    def add_medication(self, medicine_name: str, dose: str = "", schedule: str = "早餐后", notes: str = ""):
        return MedicationService(self.db, self.patient_id).create(medicine_name=medicine_name, dose=dose, schedule=schedule, notes=notes)
    ...
    def generate_report(self, report_type: str = "clinician_previsit", window_days: int = 14):
        payload = ReportRequest(report_type=report_type, window_days=window_days)
        return ReportService(self.db, self.patient_id).generate(payload)

    def get_timeline(self):
        items = PatientService(self.db).timeline(self.patient_id)
        return [{"time": i.occurred_at.isoformat(), "type": i.item_type, "title": i.title} for i in items[:10]]
