...
class WorkerBeat(Base):
    ...
    worker_name: Mapped[str] = mapped_column(String(50), unique=True, index=True)
    last_ran_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
...
def get_db():
    db = SessionLocal()
    try:
        ensure_default_patient(db)
        yield db
    finally:
        db.close()


def ensure_default_patient(db):
    if db.query(PatientProfile).count() == 0:
        db.add(PatientProfile(name='默认患者', gender='未知', diagnosis_summary='2型糖尿病合并高血压'))
        db.commit()
