...
from chronic_agent.platform.db import (
    ClinicalDigest, FollowUpVisit, HealthEvent, MealRecord, ReminderOccurrence
)
from chronic_agent.platform.repositories import (
    HealthRepository, MealRepository, ReminderRepository, FollowUpRepository, DigestRepository
)
...
class UnifiedEvent(BaseModel):
    ...
    id: str  # globally unique id, e.g. "health_event_1"
    patient_id: int
    event_category: str  # 'health', 'meal', 'reminder', 'digest', 'followup'
    event_type: str  # e.g. 'blood_pressure', 'fasting_glucose', 'auto_meal'
...
class TimelineEngine:
    """
    Longitudinal timeline engine for chronologically retrieving, filtering,
    and tagging patient events for digestion and evaluation.
    """
    def __init__(self, db: Session, patient_id: int):
        self.db = db
        self.patient_id = patient_id

    def extract_window(self, since: datetime, until: datetime | None = None, categories: list[str] | None = None) -> list[UnifiedEvent]:
        until = until or datetime.utcnow()
        events: list[UnifiedEvent] = []

        if not categories or 'health' in categories:
            for r in self.db.query(HealthEvent).filter(
        ...
        if not categories or 'meal' in categories:
            for r in self.db.query(MealRecord).filter(
        ...
        if not categories or 'reminder' in categories:
            # For reminder filtering by date, we use scheduled_for -> datetime
            for r in self.db.query(ReminderOccurrence).filter(
        ...
        if not categories or 'followup' in categories:
            for r in self.db.query(FollowUpVisit).filter(
                FollowUpVisit.patient_id == self.patient_id,
            ).all():
                dt = datetime.combine(r.scheduled_for, datetime.min.time())
                if since <= dt <= until:
                    events.append(UnifiedEvent(
